                                    ...
                                    <div class="pb-5" data-wizard-type="step-content" data-wizard-state="current">
										...
										<div class="my-5">
											...
											<div class="form-group row" id="tipomulti{{$i}}" style="visibility: hidden;">
												...
												<div class="col-9">
													<div class="input-group input-group-solid">
														<input type="text" name="multi{{$i}}" id="multi{{$i}}" class="form-control form-control-solid" placeholder="Escriba la respuesta correcta"/>
													</div>
												</div>
											</div>
		...
		for (let index = 1; index <= {{$preguntas}}; index++) {
			...
			$('#TipoPregunta'+index).on('change', function() {
				if($('#TipoPregunta'+index).val() == 'Multiple' ){
					$('#tipomulti'+index).css("visibility", "visible");
				...
				if($('#TipoPregunta'+index).val() == 'V/F' ){
					...
					$('#tipomulti'+index).css("visibility", "hidden");
					$("#P-respuestas"+index).val([]);
					$("#multi"+index).val("");
				}
		...
		function crearPreguntas(id,p)
         {
			...
			for (let i = 1; i <= p; i++) {
				...
				var Respuesta = $('#multi'+i).val();
				var PosibleR = $('#P-respuestas'+i).val();
				var RespuestaVF = $('#VF'+i).val();
				var data = [{
					Titulo: Titulo,
					Contenido: Contenido,
					Punteo: Punteo,
